...
class RoosterTeethParser:
    ...
    def _fetch_episode_links(self, season_links):
        ...
        for link in season_links:
            ...
            data = response.json()
            print(f"Parsing Season: {'Bonus Season (Checking if Exists)' if 'bonus_features' in link else season_count}...")

            for episode in data.get('data', []):
                if episode['canonical_links']['self']:
                    link = f"https://roosterteeth.com{episode['canonical_links']['self']}"
